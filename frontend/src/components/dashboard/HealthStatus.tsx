...
function StatusPill({ state, label }: { state: "healthy" | "degraded" | "unavailable"; label: string }) {
  const styles = {
  ...
  return <span className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium", styles[state])}><span className="h-1.5 w-1.5 rounded-full bg-current" />{label}</span>;
}

function Signal({ label, value, detail, state }: { label: string; value: string; detail?: string; state: "healthy" | "degraded" | "unavailable" }) {
  return <div className="rounded-lg border border-border/60 bg-muted/20 p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs text-muted-foreground">{label}</p><span className={cn("h-2 w-2 rounded-full", state === "healthy" ? "bg-emerald-500" : state === "degraded" ? "bg-amber-500" : "bg-muted-foreground")} /></div><p className="mt-1 text-sm font-medium">{value}</p>{detail && <p className="mt-0.5 text-xs text-muted-foreground">{detail}</p>}</div>;
}
